...
public static void generateOutlineexecution(List<Outlineexecution> tasklist){
	WriteDocument wd = new WriteDocument();
	wd.setVisible(true);
	wd.createNewDocument();
	wd.insertDirectory("试飞大纲执行情况");
	...
	for(int i=0;i<tasklist.size();i++){
		...
		str[5]=tasklist.get(i).getTestMethod();
		str[6]=String.valueOf(tasklist.get(i).getOutlineFlights());
		str[7]=String.valueOf(tasklist.get(i).getCombineFlights());

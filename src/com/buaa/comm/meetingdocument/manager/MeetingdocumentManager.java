		...
		if (null != details && details.size() > 0) {
	    	for(Meetingdocument item : details) {
				...
				if (state.equals(EntityState.NEW)) {
					...
					userOperationLogManager.recordUserOperationLog(0, myDate, un, "对会议材料表新增一条记录");
									} else if (state.equals(EntityState.MODIFIED)) {
					meetingdocumentDao.updateData(item);
					//对用户修改操作进行记录，在用户操作日志表中新增一条记录。
					userOperationLogManager.recordUserOperationLog(1, myDate, un, "对会议材料表修改一条记录");
									} else if (state.equals(EntityState.DELETED)) {
					meetingdocumentDao.deleteData(item);
					//对用户删除操作进行记录，在用户操作日志表中新增一条记录。
					userOperationLogManager.recordUserOperationLog(2, myDate, un, "对会议材料表删除一条记录");
